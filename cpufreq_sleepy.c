/*
 *  drivers/cpufreq/cpufreq_sleepy.c
 *
...
static
#endif
struct cpufreq_governor cpufreq_gov_sleepy = {
       .name                   = "sleepy",
...
static void sleepy_late_resume(struct early_suspend *handler) {
       sleepy_suspend(0);
}

static struct early_suspend sleepy_power_suspend = {
        .suspend = sleepy_early_suspend,
...
static void sleepy_powersave_bias_init_cpu(int cpu)
{
...
static void sleepy_powersave_bias_init(void)
{
        int i;
        for_each_online_cpu(i) {
                sleepy_powersave_bias_init_cpu(i);
        }
}
...
/* cpufreq_sleepy Governor Tunables */
                                   ...
                                   unsigned int event)
{
        ...
        case CPUFREQ_GOV_START:
                ...
                this_dbs_info->rate_mult = 1;
                sleepy_powersave_bias_init_cpu(cpu);
                /*
...
MODULE_DESCRIPTION("'cpufreq_sleepy' - A dynamic ondemand based cpufreq governor for "
        "Low Latency Frequency Transition capable processors optimized for mobile devices");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_SLEEPY
